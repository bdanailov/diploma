...
Вече сме задали метод на обучение на мрежата. Време е да изпълним самото обучение. Както е показано фигура 3.13 , избираме от *Task manager* -> *Perform training*

| ![nn-13.png](images/nn-13.png) |
|:--:|
| *Фиг.3.13* |

След изпълнението на задачата, софтуера ни предоставя информация за това какъв метод сме задали, по какъв начин се е променила средната ни грешка и загубата ни ( фиг.3.14 )

| ![nn-14.png](images/nn-14.png) |
|:--:|
| *Фиг.3.14* |

### Оптимизация

Архитектурата на невронната мрежа е от решаващо значение за доброто качество на крайния модел. Лошият модел на проблема може да доведе до големи грешки в прогнозите за нови случаи.

Дефинираме грешката за избор като грешка на невронна мрежа за нови данни. Той измерва способността на модела да предскаже резултата в нов случай.

#### Избор на входни данни

Изборът на входни данни е метод за подобряване на качеството на прогнозите. Той основно се състои в извличане на подгрупата от данни , които имат по-голямо влияние върху определен физически, биологичен, социален и т.н. процес.

За тези алгоритми съществуват два различни типа класификация:

+ Методи на филтриране:
Тези алгоритми избират характеристиките по някои свойства на променливите независимо от модела.
+ Методи за обвиване:
Тези алгоритми оценяват няколко подгрупи от променливи, което позволява да се открият възможни взаимодействия между променливите.

#### Избор на подредба

Два често срещани проблема при проектирането на невронна мрежа се наричат ​​underfitting и overfitting. Най-доброто генерализиране се постига чрез използване на модел, чиято сложност е най-подходяща за постигане на адекватна пригодност на данните. По този начин подчиняването се дефинира като ефект на грешка в селекцията, която се увеличава поради прекалено прост модел, докато свръхоборудването се дефинира като ефекта на грешка в селекцията, която се увеличава поради твърде сложен модел.

За да решим този проблем можем да оптимизираме сложността на модела, като използваме алгоритмите за избор на подредба.

*Task manager* -> *Perform order selection*

След като задача приключим можем да видим как се е променилата архитектурата на невронната ни мрежа

| ![nn-17.png](images/nn-17.png) |
|:--:|
| *Фиг.3.15* |

След промяната ще забележим промени и в грешките при обучение на мрежата.

| ![nn-15.png](images/nn-15.png) |
|:--:|
| *Фиг.3.16* |

| ![nn-16.png](images/nn-16.png) |
|:--:|
| *Фиг.3.17* |

След промяната на архитектурата трябва да обучим мрежата отново. Резултатите от обучението можем да видим на фигура 3.18.

| ![nn-18.png](images/nn-18.png) |
|:--:|
| *Фиг.3.18* |

Можем да забележим, че средната грешка на намаляла близо 50 пъти.


### Тестване на получената мрежа 

Тестването на генерираната и обучената мрежа може да бъде направено като изберем задачата *Calculate outputs* от *Task manager*

Отваря се нов диалог прозорец, в който можем да зададем стойностите отчетени от сензорите за дистанция - например [ 30, 23, 60 ]. При такива данни бихме очаквали невронната мрежа да изведе резултат, който да накара нашия робот да завие на дясно.

| ![nn-19.png](images/nn-19.png) |
|:--:|
| *Фиг.3.19* |

На фигура 3.20 можем да видим входните и изходните данни.

| ![nn-20.png](images/nn-20.png) |
|:--:|
| *Фиг.3.20* |

# Заключение

Невронните мрежи са отлични инструменти, приложими в
мобилни роботи, избягващи препятствия, които имат
способност за работа с неточна информация
Архитектурата на РНК-БП се изпълнява през 2007г
вградени възможности за обработка в реално време
система, генерирайки случаи на отговор, които бяха
не са взети под внимание при обучението
да бъде надежден и устойчив на грешки контрол. Превозното средство
контрол, водещ до стабилна диференциална задвижваща система
който беше използван.
Използваните сензори SRF02 са подходящи за това
приложение; въпреки това се предлагат допълнителни изследвания
интегрирайте серията SRF08, която предлага по-добра
надзор, леко калибриране и размер от
SRF02 серия. Необходимо е да се постигне още:
Надеждност при внедряването и управлението на
дизайн на невронни мрежи.
За по-модерни приложения се препоръчва
да работят с DSP. Използването на техники, които
включват повече време заедно сензори, такива сензори
като жироскопи, електронен компас и др
енкодер, за да се гарантира позицията на подвижната
в пространството се предлага. Използване на по-голяма близост
сензори, поне пет или шест ще ни дадат по-добра
- разбиране на околната среда, доколкото
DSP поддържа изчислителната тежест на нервните
мрежата, която ще бъде генерирана.
